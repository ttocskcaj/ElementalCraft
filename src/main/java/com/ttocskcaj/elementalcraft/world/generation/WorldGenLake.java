...
public class WorldGenLake extends WorldGenerator {
    ...
    private final Block surround;

    public WorldGenLake(Block blockIn, Block surround) {
        this.block = blockIn;
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position) {
        ...
        if (position.getY() <= 4) {
            ...
            boolean[] aBool = new boolean[2048];
            ...
            for (int k1 = 0; k1 < 16; ++k1) {
                for (int l2 = 0; l2 < 16; ++l2) {
                    for (int k = 0; k < 8; ++k) {
                        boolean flag = !aBool[(k1 * 16 + l2) * 8 + k] && (k1 < 15 && aBool[((k1 + 1) * 16 + l2) * 8 + k] || k1 > 0 && aBool[((k1 - 1) * 16 + l2) * 8 + k] || l2 < 15 && aBool[(k1 * 16 + l2 + 1) * 8 + k] || l2 > 0 && aBool[(k1 * 16 + (l2 - 1)) * 8 + k] || k < 7 && aBool[(k1 * 16 + l2) * 8 + k + 1] || k > 0 && aBool[(k1 * 16 + l2) * 8 + (k - 1)]);
            ...
            for (int i2 = 0; i2 < 16; ++i2) {
                for (int j3 = 0; j3 < 16; ++j3) {
                    for (int j4 = 4; j4 < 8; ++j4) {
                        if (aBool[(i2 * 16 + j3) * 8 + j4]) {
                            BlockPos blockpos = position.add(i2, j4 - 1, j3);
                            worldIn.setBlockState(blockpos, this.surround.getDefaultState(), 2);
                ...
                for (int j2 = 0; j2 < 16; ++j2) {
                    for (int k3 = 0; k3 < 16; ++k3) {
                        for (int k4 = 0; k4 < 8; ++k4) {
                            boolean flag1 = !aBool[(j2 * 16 + k3) * 8 + k4] && (j2 < 15 && aBool[((j2 + 1) * 16 + k3) * 8 + k4] || j2 > 0 && aBool[((j2 - 1) * 16 + k3) * 8 + k4] || k3 < 15 && aBool[(j2 * 16 + k3 + 1) * 8 + k4] || k3 > 0 && aBool[(j2 * 16 + (k3 - 1)) * 8 + k4] || k4 < 7 && aBool[(j2 * 16 + k3) * 8 + k4 + 1] || k4 > 0 && aBool[(j2 * 16 + k3) * 8 + (k4 - 1)]);

                            if (flag1 && (k4 < 4 || rand.nextInt(2) != 0) && worldIn.getBlockState(position.add(j2, k4, k3)).getMaterial().isSolid()) {
                                worldIn.setBlockState(position.add(j2, k4, k3), this.surround.getDefaultState(), 2);
                            }
                        }
                    }
                }
                ...
                for (int k2 = 0; k2 < 16; ++k2) {
                    for (int l3 = 0; l3 < 16; ++l3) {
                        ...
                        if (worldIn.canBlockFreezeWater(position.add(k2, 4, l3))) {
                            ...
                            worldIn.setBlockState(position.add(k2, 4, l3), this.surround.getDefaultState(), flag); //Forge
                        }
                    }
                }
            }
